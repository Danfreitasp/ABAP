...
*    ENDIF.
  ...
  METHOD fill_colors.
    ...
    DATA(ls_color_warning) = VALUE lvc_s_colo( col = 3 int = 0 inv = 0 ).
    DATA(ls_color_red)     = VALUE lvc_s_colo( col = 6 int = 0 inv = 0 ).
    ...
    LOOP AT lt_sflight ASSIGNING FIELD-SYMBOL(<fs_sflight>).
      ...
      ELSEIF lv_percent_occuped >= 80.
        APPEND VALUE lvc_s_scol( fname = '' color = ls_color_red nokeycol = '' ) TO <fs_sflight>-color.
      ENDIF.
    ENDLOOP.
